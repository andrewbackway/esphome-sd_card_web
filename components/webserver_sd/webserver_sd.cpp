...
// Assuming necessary includes for Path, sd_mmc, AsyncWebServerRequest, etc., are already present.
...
void SDFileServer::append_json_row(std::string &json, bool &first, const sd_mmc::FileInfo &info) const {
  if (!first) {
    json += ",\n";
  }
  first = false;
...
bool Path::trailing_slash(const std::string &path) { return !path.empty() && path.back() == separator; }

std::string Path::join(const std::string &first, const std::string &second) {
  std::string result = first;
  if (!trailing_slash(first) && !is_absolute(second)) result.push_back(separator);
